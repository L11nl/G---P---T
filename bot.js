/*
 * ==========================================================
 * ChatGPT Bot Creator - الاصدار 27 (تم إصلاحه)
 * ==========================================================
 * - تم إصلاح خطأ تعريف bot قبل الاستخدام.
 * - تم تحسين استقرار السيلكتورات.
 * - Mail.tm تلقائي + يدوي.
 * - دعم Plus (اختياري عبر متغيرات البيئة).
 * ==========================================================
 */
...
// ============================================================
// 1. التهيئة الأساسية (يجب أن تكون قبل تعريف البوت)
// ============================================================
...
// ============================================================
// 2. تعريف البوت (الآن bot معرف عالمياً)
// ============================================================
...
function generateUserInfo() {
    ...
    return {
        ...
        month,
        day,
        birthdayString: month + day + year, // MMDDYYYY
    };
}

// ============================================================
// 5. دالة إنشاء الحساب الرئيسية (نسخة 24 المدمجة مع تحسينات 27)
// ============================================================
    ...
    for (let emailAttempt = 1; emailAttempt <= maxEmailAttempts; emailAttempt++) {
        ...
        try {
            ...
            if (isManual) {
                ...
                code = await waitForMailTmCode(mailToken, chatId, 100);
            ...
            if (await nameInputNode.isVisible({ timeout: 15000 }).catch(() => false)) {
                ...
                await nameInputNode.fill(userInfo.name);
                await sleep(1000);
                
                const birthdayString = userInfo.birthdayString; 
                
                // البحث عن spinbutton الخاص بالشهر
            ...
            if (page.url().includes('/chat')) {
                 ...
                 // تفعيل Plus إذا كان مفعلاً (يمكن تعطيله بحذف متغيرات CARD)
                 if (ENABLE_PLUS) {
                     await bot.sendMessage(chatId, "💳 جاري محاولة تفعيل Plus...");
                     // يمكنك إضافة دالة subscribePlus هنا إذا أردت
                 }
            } else {
        ...
        if (!shouldRetryNewWithEmail) return false; 
    }
...
bot.onText(/\/accounts/, (msg) => {
    ...
    if (!fs.existsSync(file)) return bot.sendMessage(msg.chat.id, '📭 لا توجد حسابات.');
...
console.log("🤖 البوت يعمل (الاصدار 27 - تم إصلاح الخطأ)...");
